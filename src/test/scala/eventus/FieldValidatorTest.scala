...
import zio.Scope
import zio.test.{Gen, TestEnvironment, ZIOSpecDefault, ZSpec, assertTrue, check}
...
object FieldValidatorTest extends ZIOSpecDefault {
  ...
  private val randomIntGen = Gen.fromRandom(
    _.nextIntBetween(
      Int.MinValue + 1,
      Int.MaxValue
    )
  ...
  override def spec: ZSpec[TestEnvironment with Scope, Any] =
    suite("validators for fields tests")(
      ...
      suite("validateIntMax")(
        test("it should return result for int when it not more than max") {
          check(randomIntGen) { int =>
            assertTrue(
              validateIntMax(
                int,
                "int",
